#include "models/Positrons.h"

#include <plog/Log.h>

namespace STATS {

Positrons::Positrons(const std::string initFilename, const int& nSample,
                     std::shared_ptr<SecondaryPositrons> secondary) {
    auto positrons = std::make_shared<Particle>(initFilename, nSample);
    m_E = positrons->getE();
    m_positronSample = positrons->getSample();
...
void Positrons::init() {
    for (int i = 0; i < m_E.size(); ++i) {
        std::vector<double> v;
        for (const auto realization : m_positronSample) v.push_back(realization.second.at(i));
